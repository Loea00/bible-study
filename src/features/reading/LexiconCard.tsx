...
import type { StrongsLexicon as LexiconEntry } from '../../types/db'
...
export function LexiconCard({ strongsIds, onClose }: LexiconCardProps) {
  ...
  const [concordanceFor, setConcordanceFor] = useState<{ id: string; lemma: string } | null>(null)
  ...
  return (
    <Fragment>
        ...
        <div className="lexicon-card" onClick={(e) => e.stopPropagation()}>
          ...
          {entries?.map((entry) => (
            <div key={entry.strongs_id} className="lexicon-entry">
              ...
              <button
                type="button"
                className="lexicon-occurrences-link"
                onClick={() => setConcordanceFor({ id: entry.strongs_id, lemma: entry.lemma })}
              >
                Other occurrences →
              </button>
            </div>
      ...
      {concordanceFor && (
        <ConcordanceView
          strongsId={concordanceFor.id}
          lemma={concordanceFor.lemma}
          onClose={() => setConcordanceFor(null)}
        />
      )}
    </Fragment>
  )
